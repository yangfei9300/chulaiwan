<template>
	<view class="content">
		...
		<view class="btn" @click="xuanding(list1.name,list1.shop_no)">选定商家</view>
...
<script>
	...
	export default {
		...
		methods: {
			xuanding(name,shop_no){
				let page =getCurrentPages();
				let bpage=page[page.length-3];
				console.log(page);
				uni.navigateBack({
					delta: 2,
					success: function() {
						bpage.$vm.address = name;
						bpage.$vm.shop_no=shop_no;
						//触发上个页面的方法
					}
					// success:()=>{
					// 	bpage.$vm.address=name;
					// 	console.log(bpage.$vm.address);
					// }
				});
			},
			// getlocation(){
