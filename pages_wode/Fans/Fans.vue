		...
		<view class="Follow" v-for="(item,index) in list" :key="index">
			<view class="FollowLeft">
				...
				<view class="FollowLeftName">
					...
					class="w-86 h-40 m-left-24"  v-if="item.is_vip==1"
					></image>
					<image src="https://ta.imjiayi.com/upload/upload/huiyuan/Component 298@2x.png"
					class="w-86 h-40 m-left-24"   v-if="item.is_vip==2"
					></image>
		...
		methods: {
			...
			NoFollow(id, index) {
				...
				uni.request({
					...
					success: res => {
						if(res.data.data==1){
							this.list[index].follow_status=1
						}else{
								this.list[index].follow_status=0
						}
