<template>
	<view class="content">
		...
		<view class="address1" v-show="showwords">
			<view class="address2" v-for="(item,index) in list" :key="index"
				@click="POI(item.name,item.latitude,item.longitude,item.address)">
		...
		<view class="address1" v-show="!showwords">
			<view class="address2" v-for="(item,index) in wordslist" :key="index"
				@click="POI(item.name,item.location.lat,item.location.lng,item.address)">
				<view class="address3">{{item.name}}</view>
				<view class="address4">{{item.address}}</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			POI(name, latitude, longitude,address) {
				...
				let propages = pages[pages.length - 2];
