	...
	<view class="content">
		...
		<view class="">
			<view class="search1" @click="Address()">
				...
				<!-- @click="Address" -->
				<view class="search3">
					<!-- <image class="qhv3" src="../../static/qiehuan.png" mode="" @click="styleqh" v-if="show2">
					</image>
					<image class="qhv3" src="../../static/qiehuan.png" mode="" @click="styleqh1" v-else></image> -->
			...
			<view class="qhv1">
				<scroll-view class="scroll-view_H" scroll-x="true" scroll-left="0">
					...
					<view v-for="(item,index) in list11" :key="index" :class="{active:current===item.id}"
						@click="scrollindex(item.id,1)" id="demo1" class="scroll-view-item_H uni-bg-red">
						{{item.name}}
					</view>
			...
			<view class="" v-show="hdong">
				<view class="" v-show="show2">
					<view :class="[{'hengbanvip':item.vip_code == 1},'achd']" v-for="(item,index) in list111"
						:key="index" @click="ActiveDetail(item.act_no)">
						...
						<view class='suolue'>
							<image class="achd1" :src="item.cover+'?x-oss-process=image/auto-orient,1/resize,p_45/quality,Q_20'"
							...
							<view class="achd11">
								<view class="achd12">
									<image v-for="(items,indexs) in item.group" :key="indexs" v-if="indexs<7"
										class="achd13" :src="items.headimg+'?x-oss-process=image/auto-orient,1/resize,p_45/quality,Q_5'" mode="">
									</image>
								</view>
				...
				<view class="" v-show="show1">
					<view class="wall">
						<view class="left">
							<view class=" text-center" v-for="(item, index) in list111" :key="index" v-if="index%2==0">
								<view class="left_zview">
									<view class="shubanvipb" v-if="item.vip_code==1">
										<image class="shubanvip1" v-if='item.tag=="已满员"'
											src="../../static/index/manyuan.png" mode=""></image>
										<image class="shubanvip1" v-else src="../../static/index/vip1.png" mode="">
										</image>
									</view>
									<view v-else>
										<image class="tag" v-if="item.tag=='去组队'" src="../../static/zudui.png" mode="">
										</image>
										<image class="tag" v-if="item.tag=='已满员'" src="../../static/manyuan.png"
											mode=""></image>
									</view>

									<image class="sj_left_img" :src="item.cover" mode="aspectFill"></image>
									<view :class="{popvip:item.vip_code==1,pop:item.vip_code==0}">
										<image style="width: 40rpx;height: 40rpx;" v-if='!item.vip_code'
											src="../../static/index/naozhong.png" mode=""></image>
						...
						<view class="left">
							<view class=" text-center" v-for="(item, index) in list111" :key="index" v-if="index%2==1">
								<view class="left_zview">
									<view class="shubanvipb" v-if="item.vip_code==1">
										<image class="shubanvip1" v-if='item.tag=="已满员"'
											src="../../static/index/manyuan.png" mode=""></image>
									...
									<image class="wall_right_img" :src="item.cover" mode="aspectFill"></image>
									<view :class="{popvip:item.vip_code==1,pop:item.vip_code==0}">
										<image style="width: 40rpx;height: 40rpx;" v-if='!item.vip_code'
											src="../../static/index/naozhong.png" mode=""></image>
										<view class="" style="display: flex;">
					...
					<view class="poup3">
						<view class="poup4">
							...
							<view class="" style="display: flex;flex-direction: column;">
								...
								<view class="poup5">
									<block v-if="sjia">
											<view class="poup6" @click="numpaixu('')" :class="{actives:numbee===''}">默认</view>
											<view class="poup6" @click="numpaixu('1')" :class="{actives:numbee==='1'}">离我最近</view>
											<view class="poup6" @click="numpaixu('2')" :class="{actives:numbee==='2'}">好评优先</view>
									</block>
									<block v-if="hdong">
										<view class="poup6" @click="numpaixu('')" :class="{actives:numbee===''}">默认</view>
										<view class="poup6" @click="numpaixu('1')" :class="{actives:numbee==='1'}">离我最近</view>
										<view class="poup6" @click="numpaixu('2')" :class="{actives:numbee==='2'}">最新发布</view>
										<view class="poup6" @click="numpaixu('3')" :class="{actives:numbee==='3'}">最近场次</view>
						...
						<view class="poup4">
							...
							<view class="" style="display: flex;flex-direction: column;">
								...
								<view class="poup5">
									...
									@click.stop="selQuanInfoCLick(null)"
									>不限</view>
									<view class="poup6"  :class="{actives:selQuanInfo}"
		...
		<view class="huuibeijing" style="z-index: 5000;" v-if="shangquanAlert">
			<view class="bottomquanBigview colonn">
				<view class="rowsb" style="padding:30rpx;">
					<image src="https://ta.imjiayi.com/upload/upload/关闭.png" class="guanbiicon yincang1"></image>
					<view>热门商圈</view>
					<image src="https://ta.imjiayi.com/upload/upload/关闭.png" @click.stop="shangquanAlertClick" class="guanbiicon"></image>
				</view>
				<scroll-view scroll-y style="width: 750rpx;height: 700rpx;">
					<view class="huanhang rowsb" style="padding:30rpx;">
						...
						@click.stop="selQuanInfoCLick(item)"
						>{{item.name}}</view>
					</view>
	...
	export default {
		...
		watch: {
			searchs(newval, oldval) {
				uni.request({
					url: this.baseurl + '/api/act/actList',
					method: 'POST',
					header: {
						xcx: 1,
						token: this.token
				...
				if (this.numbee == 2) {
					uni.request({
						...
						data: {
							...
							money_max: this.max,
							money_min: this.min,
							business_area:this.selQuanInfo?this.selQuanInfo.id:''
						},
						success: res => {
							console.log("活动列表", res);
							this.list111 = res.data.data;
